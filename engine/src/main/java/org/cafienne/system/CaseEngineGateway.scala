...
class CaseEngineGateway(caseSystem: CaseSystem) {
  ...
  def request(family: RootIdentifier, message: CaseEngineCommand): Future[ModelResponse] = {
  ...
  def inform(family: RootIdentifier, message: CaseEngineCommand, replyTo: ActorRef = ActorRef.noSender): Unit = {
    getRouter(message).tell(message, replyTo)
  }

  def terminate(family: RootIdentifier, actorId: String): Unit = {
  ...
  private def getRouter(message: CaseEngineCommand): ActorRef = {
      val actorClass = message.actorType.actorClass

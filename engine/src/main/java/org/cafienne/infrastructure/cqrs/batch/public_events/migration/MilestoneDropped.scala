...
object MilestoneDropped {
  def from(batch: PublicCaseEventBatch): Seq[PublicEventWrapper] = batch
    ...
    .map(event => PublicEventWrapper(batch.timestamp, batch.getSequenceNr(event), MilestoneDropped(event.getPlanItemId, event.stageId, event.path, event.path.name, event.getCaseInstanceId)))

  def deserialize(json: ValueMap): MilestoneDropped = MilestoneDropped(

...
@Manifest
case class StageDropped(stageId: String, parentStage: String, path: Path, name: String, caseInstanceId: String) extends CafiennePublicEventContent {
...
object StageDropped {
  def from(batch: PublicCaseEventBatch): Seq[PublicEventWrapper] = batch
    .filterMap(classOf[PlanItemDropped])
    .filter(_.getType.isStage)
    .map(event => PublicEventWrapper(batch.timestamp, batch.getSequenceNr(event), StageDropped(event.getPlanItemId, event.stageId, event.path, event.path.name, event.getCaseInstanceId)))

  def deserialize(json: ValueMap): StageDropped = StageDropped(

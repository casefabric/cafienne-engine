...
@Manifest
case class MilestonePending(milestoneId: String, parentStage: String, path: Path, name: String, caseInstanceId: String) extends CafiennePublicEventContent {
...
object MilestonePending {
  def from(batch: PublicCaseEventBatch): Seq[PublicEventWrapper] = batch
    ...
    .map(event => PublicEventWrapper(batch.timestamp, batch.getSequenceNr(event), MilestonePending(event.getPlanItemId, event.stageId, event.path, event.path.name, event.getCaseInstanceId)))

  def deserialize(json: ValueMap): MilestonePending = MilestonePending(

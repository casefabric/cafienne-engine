...
trait CommandRoute extends AuthenticatedRoute {
  ...
  def askCaseEngine(command: CaseCommand): Route = CommandRouteExecutor.askModelActor(command, caseSystem.engine.request(RootIdentifier(command.actorId), command))

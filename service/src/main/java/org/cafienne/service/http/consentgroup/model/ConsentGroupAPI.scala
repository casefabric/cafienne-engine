...
object ConsentGroupAPI {
  ...
  case class ConsentGroupFormat(
    ...
    PayloadValidator.runDuplicatesDetector("Consent group", "user", members.map(_.userId))
    PayloadValidator.requireElements(members, "Setting consent group requires a list of users with at least one owner")
  ...
  case class ConsentGroupUserFormat(
    ...
    PayloadValidator.required(userId, "Consent group users must have a userId")
